...
bool Recorder::start(HWND hwnd){
    ...
    if(m_audioOn){
        ...
        m_audioTmpBase = base+".audio";
        m_audioTmpMix  = m_audioTmpBase+".mix.wav";
        m_audioTmpGame = m_audioTmpBase+".game.wav";
        m_audioTmpMic  = m_audioTmpBase+".mic.wav";
    } else {
        m_videoTmpFile = m_outFile;     // ffmpeg writes straight to the final file
        m_audioTmpBase.clear(); m_audioTmpMix.clear(); m_audioTmpGame.clear(); m_audioTmpMic.clear();
    }
...
void Recorder::finalizeStop(){
    ...
    if(m_videoTmpFile!=m_outFile){
        std::error_code ec;
        auto rmWavs=[&]{
            for(const std::string* w : {&m_audioTmpMix,&m_audioTmpGame,&m_audioTmpMic})
                if(!w->empty()) std::filesystem::remove(std::filesystem::path(w2(*w)),ec);
        };
        bool muxed=false;
        if(m_audioOn){
...
bool Recorder::runMux(){
    if(m_videoTmpFile.empty()){setLastError("mux: missing temp");return false;}
    std::error_code ec;
    if(!std::filesystem::exists(std::filesystem::path(w2(m_videoTmpFile)),ec)){setLastError("mux: no video");return false;}

    // Inputs in a FIXED order so OUTPUT audio-stream indices are stable:
    //   a:0 = mix (Game + Mic) -> default track, a:1 = game, a:2 = mic.
    struct Track { const std::string* path; const char* title; };
    const Track wanted[3] = {
        { &m_audioTmpMix,  "Game + Mic (Mix)" },
        { &m_audioTmpGame, "Game" },
        { &m_audioTmpMic,  "Mic" },
    };
    std::vector<Track> present;
    for(const auto& t : wanted)
        if(!t.path->empty() && std::filesystem::exists(std::filesystem::path(w2(*t.path)),ec))
            present.push_back(t);

    std::ostringstream cmd;
    cmd << q(ffPath()) << " -hide_banner -loglevel error -y -i " << q(m_videoTmpFile) << ' ';
    for(const auto& t : present) cmd << "-i " << q(*t.path) << ' ';
    cmd << "-map 0:v:0 ";
    for(size_t i=0;i<present.size();++i) cmd << "-map " << (i+1) << ":a:0 ";
    // No "+faststart": moov-at-end plays fine locally and avoids a second full
    // read+write pass that would double mux time on a long recording.
    cmd << "-c:v copy ";
    if(!present.empty()) cmd << "-c:a aac -b:a " << m_audioBitrate << "k ";
    for(size_t i=0;i<present.size();++i)
        cmd << "-metadata:s:a:" << i << ' ' << q(std::string("title=")+present[i].title)
            << " -metadata:s:a:" << i << " language=eng ";
    for(size_t i=0;i<present.size();++i)
        cmd << "-disposition:a:" << i << ' ' << (i==0 ? "default" : "0") << ' ';
    cmd << "-shortest " << q(m_outFile);
